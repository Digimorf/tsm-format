...
REM ============================================================
REM build_v5_2_compact.bat
REM
REM Complete TSM v5.2 compact escape toolchain build.
REM ============================================================
...
echo Building TSM v5.2 compact escape toolchain...

echo.
echo [1/8] Compiling tsm_v5.c
gcc %CFLAGS% -c "%SRC_DIR%\tsm_v5.c" -o "%OBJ_DIR%\tsm_v5.o"
if errorlevel 1 goto build_error

echo.
echo [2/8] Compiling wav_io_simple.c
gcc %CFLAGS% -c "%SRC_DIR%\wav_io_simple.c" -o "%OBJ_DIR%\wav_io_simple.o"
if errorlevel 1 goto build_error

echo.
echo [3/8] Compiling indexed_common.c
gcc %CFLAGS% -c "%SRC_DIR%\indexed_common.c" -o "%OBJ_DIR%\indexed_common.o"
if errorlevel 1 goto build_error

echo.
echo [4/8] Linking wav_tsm_advisor_v5_indexed.exe
gcc %CFLAGS% ^
...
echo.
echo [5/8] Linking wav2tsm_indexed_v5.exe
gcc %CFLAGS% ^
...
echo.
echo [6/8] Linking tsm2wav_v5.exe
gcc %CFLAGS% ^
    "%SRC_DIR%\tsm2wav_v5.c" ^
    "%OBJ_DIR%\tsm_v5.o" ^
    "%OBJ_DIR%\wav_io_simple.o" ^
...
echo.
echo [7/8] Linking wav_gap_scan.exe
gcc %CFLAGS% ^
...
echo.
echo [8/8] Linking tsm_v5_indexed_audit.exe
gcc %CFLAGS% ^
